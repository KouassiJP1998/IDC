...
window.onload = function(){
  ...
  for (var i = 0; i < tablePlaceName.length; i++) {
        tablePlaceName[i].calculBarycentre();
  }
...
function parseToXML2(){
  ...
  for (var i = 0; i < tablePlaceName.length; i++) {
      ...
      placeNameElem.setAttribute("key",placeNameAdd.keyAffichage);
      ...
      //si il y a un barycentre
      if (placeNameAdd.barycentre.latitude != null){

        var barycentreElement = doc.createElement("barycentre");

        var pointElement = doc.createElement("point");
        if (placeNameAdd.barycentre.nom != "unknown") {
            pointElement.setAttribute("name",placeNameAdd.barycentre.nom);
        }
        if (placeNameAdd.barycentre.place != "unknown") {
            pointElement.setAttribute("place",placeNameAdd.barycentre.place);
        }
        if (placeNameAdd.barycentre.altitude != "unknown") {
            pointElement.setAttribute("altitude",placeNameAdd.barycentre.altitude);
        }

        pointElement.setAttribute("latitude",placeNameAdd.barycentre.latitude);
        pointElement.setAttribute("longitude",placeNameAdd.barycentre.longitude);

        barycentreElement.appendChild(pointElement);
        placeNameElem.appendChild(barycentreElement);
      }
