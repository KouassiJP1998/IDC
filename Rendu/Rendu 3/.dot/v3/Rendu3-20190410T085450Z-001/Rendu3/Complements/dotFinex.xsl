...
digraph {
    ...
    <xsl:template match="*[name() = 'liste']/*[name()='lieu']" mode="etape2"> 
        ...
        <xsl:if test="direct-edge/@id !=''">
            <xsl:value-of select="@id"/> -> <xsl:value-of select="direct-edge/@id"/>;   
        </xsl:if>
    ...
    <xsl:template match="*[name() = 'infoPlace']">       
        <xsl:if test="@id !=''">
            <xsl:value-of select="parent::node()/@id"/> -> <xsl:value-of select="@id"/> [style=dotted dir=none][label= test fontcolor=cadetblue];  
        </xsl:if>    
    </xsl:template>
