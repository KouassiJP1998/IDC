...
<!--
Dans Prolognan.xml on extrait toute les balises utilent; on créer un fichier todot.xml
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" >
    ...
    <xsl:template match="*[name()='phr']">
        <xsl:element name="{local-name()}" xmlns="http://www.tei-c.org/ns/1.0">
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates select=" *[name()='name'] | *[name()='placeName'] | *[name()='w']"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="*[name()='placeName']">
        <xsl:element name="{local-name()}" xmlns="http://www.tei-c.org/ns/1.0">
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates select="*[name()='name'] | *[name()='w']  | *[name()='placeName']"/>
        </xsl:element>
    </xsl:template>
